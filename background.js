...
const MAX_PERSISTED_CACHE_ENTRIES = 3000;
...
async function ensureCacheLoaded() {
  ...
  cacheLoaded = true;
  try {
    const stored = await browser.storage.local.get(PERSISTED_CACHE_KEY);
    const entries = Array.isArray(stored?.[PERSISTED_CACHE_KEY]) ? stored[PERSISTED_CACHE_KEY] : [];
...
function persistCacheSoon() {
  ...
  cachePersistTimer = setTimeout(async () => {
    ...
    try {
      const entries = Array.from(translationCache.entries()).slice(-MAX_PERSISTED_CACHE_ENTRIES);
      await browser.storage.local.set({ [PERSISTED_CACHE_KEY]: entries });
    } catch (error) {
...
async function translateBatch({ texts, targetLanguage }) {
  ...
  for (let i = 0; i < segments.length; i += 1) {
    ...
    if (translationCache.has(key)) {
      ...
      resolved[i] = { id: segId, text: cached };
    } else {
  ...
  for (let idx = 0; idx < missing.length; idx += 1) {
    ...
    setCacheValue(makeCacheKey(settings.model, targetLanguage, sourceText), translatedText);
  }
