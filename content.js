...
async function translateNodes(nodes) {
  ...
  try {
    ...
    if (STATE.rescanRequested) {
      ...
      if (remaining > 0) {
        const madeProgress =
          STATE.lastRemainingCount === null || remaining < STATE.lastRemainingCount;
        if (madeProgress) {
          STATE.stagnantLoops = 0;
          scheduleTranslation();
        } else {
          STATE.stagnantLoops += 1;
          if (STATE.stagnantLoops <= 1) {
            scheduleTranslation();
          }
        }
