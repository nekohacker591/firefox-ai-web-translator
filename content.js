  ...
  while (current) {
    if (shouldTranslateNode(current)) {
      ...
      if (!alreadyTranslatedForTarget) {
        const retryMeta = STATE.nodeRetryMeta.get(current);
        if (retryMeta && retryMeta.nextRetryAt && Date.now() < retryMeta.nextRetryAt) {
          current = walker.nextNode();
          continue;
        }
...
function findUntranslatedCandidates(limit = 120) {
  const candidates = [];
  const roots = getTraversalRoots(document.body);

  for (const root of roots) {
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, null);
    let current = walker.nextNode();
    while (current && candidates.length < limit) {
      if (shouldTranslateNode(current)) {
        const rawText = current.nodeValue || "";
        const hasNonLatin = containsNonLatinScript(rawText);
        const lastTranslated = STATE.processedNodes.get(current);
        const alreadyTranslatedForTarget =
          lastTranslated &&
          lastTranslated.targetLanguage === STATE.targetLanguage &&
          lastTranslated.text === rawText;

        if (!alreadyTranslatedForTarget) {
          const shouldForceCandidate =
            STATE.forceTranslateForHost ||
            STATE.targetLanguage.toLowerCase() !== "english" ||
            hasNonLatin;
          if (shouldForceCandidate) {
...
function updateProgressBar(pendingCount) {
  ...
  if (pendingCount === 0 && !STATE.inFlight) {
...
async function sendMessageWithTimeout(message, timeoutMs = 20000) {
  return Promise.race([
    browser.runtime.sendMessage(message),
    new Promise((_, reject) =>
      window.setTimeout(() => reject(new Error("Translation request timed out.")), timeoutMs)
    )
  ]);
}
...
function buildBatches(nodes) {
  ...
  for (let i = 0; i < nodes.length; i += BATCH_SIZE) {
    ...
    requests.push({ batch, segments, texts: segments.map((segment) => segment.text) });
  }
  return requests;
}
...
async function translateNodes(nodes) {
  ...
  const requests = buildBatches(nodes);
  let processedRequestCount = 0;

  try {
    const parallel = Math.min(5, Math.max(1, STATE.batchParallel || 2));
    for (let i = 0; i < requests.length; i += parallel) {
      const wave = requests.slice(i, i + parallel);
      const waveResults = await Promise.all(
        wave.map(async (req) => {
          const result = await sendMessageWithTimeout({
            type: "TRANSLATE_BATCH",
            payload: {
              texts: { segments: req.segments },
      ...
      for (const { req, result } of waveResults) {
        ...
        const translations = req.segments.map(
          (segment, index) =>
            translatedById.get(segment.id) || result?.translations?.[index] || req.texts[index]
        );

        for (let index = 0; index < req.batch.length; index += 1) {
          const node = req.batch[index];
          if (!node || !node.isConnected) {
            continue;
          }

          if (!STATE.originalText.has(node)) {
            STATE.originalText.set(node, req.texts[index]);
          }

          const translated = translations[index] || req.texts[index];
          node.nodeValue = translated;
          if (shouldMarkAsTranslated(req.texts[index], translated)) {
            STATE.nodeRetryMeta.delete(node);
            ...
            const prev = STATE.nodeRetryMeta.get(node) || { count: 0, nextRetryAt: 0 };
            const count = prev.count + 1;
            const backoffMs = Math.min(30000, 1000 * 2 ** Math.min(5, count - 1));
            STATE.nodeRetryMeta.set(node, {
              count,
      ...
      const remainingRequests = Math.max(0, requests.length - processedRequestCount);
      updateProgressBar(remainingRequests * BATCH_SIZE);
    }
  } catch (error) {
...
function restoreOriginalText() {
  ...
  STATE.nodeRetryMeta = new WeakMap();
  updateProgressBar(0);
}
...
function startPassiveRescan() {
  if (STATE.passiveRescanIntervalId) {
    window.clearInterval(STATE.passiveRescanIntervalId);
  }

  STATE.passiveRescanIntervalId = window.setInterval(() => {
    if (!STATE.enabled || document.visibilityState !== "visible") {
      return;
    }

    const misses = findUntranslatedCandidates(80);
    if (misses.length > 0) {
...
function stopPassiveRescan() {
  if (STATE.passiveRescanIntervalId) {
    window.clearInterval(STATE.passiveRescanIntervalId);
    STATE.passiveRescanIntervalId = null;
  }
}
...
async function applySettings(settings) {
  ...
  STATE.batchParallel = Math.min(5, Math.max(1, Number.parseInt(String(settings.batchParallel || 2), 10) || 2));
